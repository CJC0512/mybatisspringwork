...
public class OrderService {
    ...
    *   현재의 경우 사용자가 고른 메뉴들 각각의 코드 번호와 고른 메뉴 갯수, 그리고 서버의 현재 시간이
    *   담긴 채로 넘어왔다는 생각을 가지고 개발해 나가자.
    * */

    /* 설명. 서비스 메소드 내부에서 모든 DML관련 작업에 예외 없이 잘 동작하면 commmit, 에외가 발생하면 rollbakc이 적용된다. */
    /* 설명.
     *  전파행위 옵션
     *   -
     ...
     *                   (모든 동시성 문제 발생) - default
     *  READ_COMMITTED : 트랜젝션이 다른 트랜젝션에서 커밋한 값만 읽을 수 있다.
     *                   오염된 값 읽기 문제는 해결되지만 재현 불가능한 값 읽기 및 허상읽기는 여전히 발생할 수 있다.
     *                   (다른 로우 수정 및 추가는 막을 수 없다.)
...
//        System.out.println("menuCode = " + menuCode);
        ...
        int totalOrderPrice = clacTotalOrderPrice(orderInfo.getOrderMenus(), menus);
...
//        );
    ...
    private int clacTotalOrderPrice(List<OrderMenuDTO> orderMenus, List<Menu> menus) {
