...
public class MybatisConfiguration {
    ...
    public HikariDataSource dataSource() {
        ...
        /* 설명. 데이터베이스 연결이 유휴상태로 유지될 수 있는 최대 시간 설정(쿼리를 보내거나 데이터를 업데이트 하지 않는 시간 설정) */
        dataSource.setIdleTimeout(600000);  // 10분

        /* 설명. 데이터베이스 연결이 최대로 유지될 수 있는 시간 설정 */
    ...
    public SqlSessionFactory sqlSessionFactory() throws Exception {
        ...
        factoryBean.setDataSource(dataSource());            // 접속할 DB 정보
        factoryBean.setConfiguration(configuration);        // 활용한 mapper 정보(쿼리 정보)
